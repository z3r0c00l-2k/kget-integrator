/*
 * kget-integrator is an extension to integrate KGet Download manager
...
var kgetWrapperNotFound = true;
var disposition = '';
var hostName;
var kgetWrapperVersion;
var kgetVersion = '';
var chromeVersion;
var firefoxVersion;
...
hostName = 'com.hell.kgetchromewrapper';
...
current_browser.storage.sync.get(function(items) {
    if (items["kget-keywords-exclude"]) {
        keywordsToExclude = items["kget-keywords-exclude"].split(/[\s,]+/);
    } else {
        current_browser.storage.sync.set({ "kget-keywords-exclude": '' });
    }

    // Read the local storage for included keywords
    if (items["kget-keywords-include"]) {
        keywordsToInclude = items["kget-keywords-include"].split(/[\s,]+/);
    } else {
        current_browser.storage.sync.set({ "kget-keywords-include": '' });
    }
...
// Message format to send the download information to the kget-integrator
var message = {
    url: '',
    cookies: '',
    useragent: '',
...
current_browser.downloads.onCreated.addListener(function(downloadItem) {
    ...
    if (kgetWrapperNotFound || !interruptDownloads) { // kget-integrator not installed
        return;
    }
...
current_browser.webRequest.onHeadersReceived.addListener(function(details) {
    ...
    if (kgetWrapperNotFound) { // kget-integrator not installed
        return {
...
/**
 * Send message to the kget-integrator
 */
function sendMessageToHost(message) {
    current_browser.runtime.sendNativeMessage(hostName, message, function(response) {
        clearMessage();
        kgetWrapperNotFound = (response == null);
        if (!kgetWrapperNotFound && !kgetWrapperVersion) {
            kgetWrapperVersion = response.version;
...
function getState() {
    if (kgetWrapperNotFound || !kgetWrapperVersion) {
        return 2;
    } else if (!kgetWrapperVersion.startsWith(REQUIRED_WRAPPER_VERSION)) {
        return 1;
    } else {
        return 0;
    }
...
function updateKeywords(include, exclude) {
    ...
    current_browser.storage.sync.set({ "kget-keywords-include": include });
    current_browser.storage.sync.set({ "kget-keywords-exclude": exclude });
}

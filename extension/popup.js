...
window.addEventListener('load', function(evt) {
	...
	let interrupt = (current_browser.storage.sync["kget-interrupt"] == "true");
	document.getElementById('save').addEventListener('click', saveChanges);
	current_browser.storage.sync.get(function(items) {
		document.getElementById('keywordsToExclude').value = items["kget-keywords-exclude"];
		document.getElementById('keywordsToInclude').value = items["kget-keywords-include"];
		document.getElementById('fileSize').value = parseInt(items["kget-min-file-size"]) / 1024;
		document.getElementById('chk-interrupt').checked = items["kget-interrupt"] == "true";
	});
});